...
# Meta achievements that are not tied to a specific fish type
# Structure: achievement_key (String) -> bool
var meta_achievements = {
	"drone_lift": false
}
...
# Get all non-fish achievements
func get_meta_achievements():
	return meta_achievements

# Record usage of the selling drone achievement
func record_drone_lift_use():
	# Skip achievements during intro mission
	if GameState.is_intro():
		return

	if not meta_achievements["drone_lift"]:
		meta_achievements["drone_lift"] = true
		emit_signal("achievement_updated", "meta", "drone_lift")
...
func save_data():
	...
	for fish_type_id in fish_achievements:
		...
		if typeof(fish_type_id) == TYPE_INT or str(fish_type_id).is_valid_int():
			save_fish_ach_data[str(fish_type_id)] = fish_achievements[fish_type_id]

	return {
		"fish": save_fish_ach_data,
		"meta": meta_achievements.duplicate()
	}
...
func load_data(data):
	...
	# Backwards compatibility: old saves stored only fish achievements at root level
	if typeof(data) == TYPE_DICTIONARY and data.has("fish"):
		fish_data = data["fish"]
		meta_data = data.get("meta", {})

	if typeof(fish_data) == TYPE_DICTIONARY:
		for fish_type_str in fish_data:
			# Ignore non-numeric keys defensively
			if not str(fish_type_str).is_valid_int():
				continue
			# Convert string key back to integer
			var fish_type_id = int(fish_type_str)
			fish_achievements[fish_type_id] = fish_data[fish_type_str]

	if typeof(meta_data) == TYPE_DICTIONARY:
		if meta_data.has("drone_lift"):
			meta_achievements["drone_lift"] = bool(meta_data["drone_lift"])
